...
/// Decode one datum using a names map built once by
/// `schema::build_names`, instead of rebuilding a ResolvedSchema
/// on every call the way `decode`/`from_avro_datum` do.
pub fn decode_resolved<R: Read>(
    schema: &Schema,
    names: &crate::schema::Names,
    reader: &mut R,
) -> AvroResult<Value> {
