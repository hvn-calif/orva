...
//!
//! The recursion is supplied only by a global allocator that itself calls
//! Rust's `std::backtrace::Backtrace` on alloc (the sole thing that takes
//! `std::sys::backtrace::lock`): a Rust tracing/heap-profiler allocator
//! (`dhat`, `bytehound`, custom shims). `AllocProfiler` below models exactly
//! that. NOTE: TCMalloc does NOT qualify -- it is C++ and unwinds sampled
//! allocations with its own lock-free stack walker, never entering the Rust
//! backtrace lock, so a TCMalloc-based binary does not reproduce this.
//!
//! Modes:
