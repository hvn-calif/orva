...
run_barrier() { # reviewer's actual M2 repro, under TSAN
  echo "### Mechanism 2 (reviewer's repro): 10 barrier-synced capture()+panic!() threads"
  echo "### watch 'completed rounds' -- climbing = no hang; frozen (alive) = candidate hang -> gdb"
  RUSTFLAGS="-Zsanitizer=thread -Cunsafe-allow-abi-mismatch=sanitizer" \
    cargo "+$TC" build --quiet --target "$TRIPLE" --bin barrier_repro 2>/dev/null \
    || RUSTFLAGS="-Zsanitizer=thread" cargo "+$TC" build --quiet --target "$TRIPLE" -Zbuild-std --bin barrier_repro
  RUST_BACKTRACE=1 TSAN_OPTIONS="halt_on_error=0" "./target/$TRIPLE/debug/barrier_repro"
}
